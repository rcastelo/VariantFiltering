...
%%} else
%%  system.time(uind <- unrelatedIndividuals(vfpar)) ## ~90 sec.
%%@
...
the one controlling minor allele frequency (MAF) values:
...
MAFmask
MAFpop(uind) <- !MAFmask
MAFpop(uind, "EAS_AF1Kgenomes.phase3") <- TRUE
MAFpop(uind)
maxMAF(uind) <- 0.01
summary(uind)
@
In this case we selected variants with MAF$< 0.01$ in the east asian population of the
