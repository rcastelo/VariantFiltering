...
<<>>=
active(filters(uind)) <- FALSE
active(filters(uind))["KGp1"] <- TRUE
cutoffs(uind)$KGp1
cutoffs(uind)$KGp1$popmask
cutoffs(uind)$KGp1$value
change(cutoffs(uind)$KGp1, "popmask") <- FALSE
cutoffs(uind)$KGp1$popmask
change(cutoffs(uind)$KGp1, "popmask") <- c(KGp1AF=TRUE)
cutoffs(uind)$KGp1$popmask
change(cutoffs(uind)$KGp1, "value") <- 0.01
summary(uind)
@
